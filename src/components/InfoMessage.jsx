...
const InfoMessage = ({ messages = [] }) => {
    return (
        <>
            <div className="flex w-full cursor-pointer flex-col items-center justify-center gap-4 mobile:gap-2">
                {messages.map((msg, index) => (
                    <div
                        key={index}
                        className="w-[80%] items-center font-nunito text-3xl font-black mobile:text-base ipad:text-2xl"
                    >
                        <div className="flex w-full items-center gap-6 rounded-2xl border-8 border-moldcheese bg-white px-4 py-2 mobile:gap-2 mobile:border-4 mobile:px-2 mobile:py-1 ipad:gap-3">
