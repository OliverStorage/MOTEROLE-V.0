...
const DrawingMetrics = () => {
    ...
    const calculateMetrics = (userMask, correctMask) => {
        const intersection = userMask.reduce((acc, val, index) => acc + (val && correctMask[index] ? 1 : 0), 0);
        const union = userMask.reduce((acc, val, index) => acc + (val || correctMask[index] ? 1 : 0), 0);

        const iou = intersection / union;
        const precision = intersection / userMask.reduce((acc, val) => acc + val, 0);
        const recall = intersection / correctMask.reduce((acc, val) => acc + val, 0);
    ...
    const compareDrawings = async () => {
        ...
        const correctMask = await preprocessImage(correctImage, canvasRefCorrect);
